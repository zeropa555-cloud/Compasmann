...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (anim != null)
        {
            ...
            if (!string.IsNullOrEmpty(dashingParam)) anim.SetBool(dashingParam, isDashing);
        }
    ...
    void FixedUpdate()
    {
        if (!isDashing) rb.linearVelocity = moveInput * moveSpeed;
    }

    void LateUpdate()  // 🆕 Animasyondan sonra visual scale'i sabitle
    {
    ...
    {
        while (isDashing) { CreateGhost(); yield return new WaitForSeconds(ghostDelay); }
    }
